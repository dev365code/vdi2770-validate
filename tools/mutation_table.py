...
TABLE = [
    ...
    ("cli/one-bad-path-does-not-stop-the-rest",
     ...
     'why = e.strerror or without_addresses(str(e))',
     ["tests/test_cli.py"],
...
PLATFORM_ROWS = [
    ...
    ('gates/a-release-newer-than-anything-written-here-is-compared',
     'tools/check_paths_are_disjoint.py',
     '    return found',
     '    return found[:1]',
     ['tests/test_no_two_distributions_claim_one_path.py::test_a_release_newer_than_anything_written_here_is_compared'],
     'this gate carried the newest release as a constant, and the next release left it comparing against the one before -- green, because nothing collided with that one either'),

    ('gates/an-old-release-is-compared-too',
     'tools/check_paths_are_disjoint.py',
     '    return found',
     '    return found[-1:]',
     ['tests/test_no_two_distributions_claim_one_path.py::test_a_collision_in_an_old_release_is_still_a_collision'],
     'somebody who installed an old release upgrades with the same command as somebody on the newest, and asking only about the newest is the constant again with a fresher number in it'),

    ('gates/an-index-that-lists-no-wheel-is-a-refusal',
     'tools/check_paths_are_disjoint.py',
     '    if not found:',
     '    if False:',
     ['tests/test_no_two_distributions_claim_one_path.py::test_an_index_that_lists_no_wheel_is_a_refusal'],
     'a comparison against nothing passes: every release of that name would drop out of the comparison without a word'),

    ('gates/a-release-without-a-wheel-is-a-refusal',
     'tools/check_paths_are_disjoint.py',
     '    if missing:',
     '    if False:',
     ['tests/test_no_two_distributions_claim_one_path.py::test_a_release_the_index_lists_without_a_wheel_is_a_refusal',
      'tests/test_no_two_distributions_claim_one_path.py::test_a_release_known_only_by_its_source_archive_is_a_refusal'],
     'pip builds such a release from its source, and leaving it out makes the comparison smaller without saying so'),

    ('gates/a-download-is-the-file-the-index-published',
     'tools/check_paths_are_disjoint.py',
     '    if hashlib.sha256(data).hexdigest() != digest:',
     '    if False:',
     ['tests/test_no_two_distributions_claim_one_path.py::test_a_download_that_is_not_the_published_file_is_refused'],
     'a file that is not the one the index gave a digest for answers a question about some other archive'),

    ('gates/the-index-is-asked-not-to-answer-from-a-cache',
     'tools/check_paths_are_disjoint.py',
     '        "Cache-Control": "no-cache",',
     '        "Cache-Control": "max-age=600",',
     ['tests/test_no_two_distributions_claim_one_path.py::test_the_index_is_asked_not_to_answer_from_a_cache'],
     'an answer kept from before an upload is one release short; the request says it wants the index, and a cache on the way that honours that cannot hand back the older page (PyPI\'s own CDN does not, and the tool says so)'),

    ('gates/the-release-being-built-is-in-the-comparison',
     'tools/check_paths_are_disjoint.py',
     '        said = overlaps(wheels, served)',
     '        said = overlaps(served)',
     ['tests/test_no_two_distributions_claim_one_path.py::test_a_release_newer_than_anything_written_here_is_compared',
      'tests/test_no_two_distributions_claim_one_path.py::test_every_name_the_index_serves_is_asked'],
     'the wheels this tree builds are the half of the comparison the gate exists for; compare only what is published and every release is green, whatever it ships'),

    ('gates/every-name-the-index-serves-is-asked',
     'tools/check_paths_are_disjoint.py',
     '        for name in NAMES:',
     '        for name in NAMES[:1]:',
     ['tests/test_no_two_distributions_claim_one_path.py::test_every_name_the_index_serves_is_asked'],
     'both names have releases people installed, and a collision with a release of the old name is as destructive as one with the engine'),

    ('gates/two-published-releases-are-not-this-releases-to-answer-for',
     'tools/check_paths_are_disjoint.py',
     '        said = overlaps(wheels, served)',
     '        said = overlaps(wheels + served)',
     ['tests/test_no_two_distributions_claim_one_path.py::test_two_releases_already_published_are_not_this_releases_to_answer_for'],
     'a collision between two releases already on the index is a fact no commit can change, and counting it would fail every release after it, the repair included'),

    ('gates/a-yanked-release-is-still-compared',
     'tools/check_paths_are_disjoint.py',
     '            if not filename.endswith(".whl"):',
     '            if not filename.endswith(".whl") or entry.get("yanked"):',
     ['tests/test_no_two_distributions_claim_one_path.py::test_a_yanked_release_is_still_compared'],
     'pip still installs a yanked release when it is pinned, and whoever has it upgrades with the same command as everybody else'),

    ('gates/a-file-listed-twice-is-a-refusal',
     'tools/check_paths_are_disjoint.py',
     '            if filename in seen:',
     '            if False:',
...
LISTING_BUDGET_ROWS = [
    ...
    ("pages/a-pin-a-reader-runs-is-past-every-advisory",
     "README.md",
     'pip install "vdi2770-validate==0.10.2"',
    ...
    ("pages/an-action-ref-a-reader-copies-is-past-every-advisory",
     "README.md",
     "| `uses: dev365code/vdi2770-validate@v0.10.2` |",
    ...
    ("pages/a-pin-in-a-sentence-is-past-every-advisory",
     "README.md",
     "`vdi2770-validate==0.10.2` installs one matching pair.",
