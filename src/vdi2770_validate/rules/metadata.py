...
from ..catalog import CLASSIFICATION_SYSTEM, ISO_639_1, document_classes, english_for, german_names, rule
...
def check(container, document, is_main: bool) -> Iterator[Finding]:
    ...
    for c in vdi:
        if c.class_id not in known:
            ...
            continue
        want_de = german_names()[c.class_id]
        ...
        for lang, text in c.names:
            ...
            if low.startswith("de"):
                if text != want_de:
                    ...
                    yield Finding(r, r.title, c.src.child(container=container.path,
                                                          member=container.metadata_name),
                                  detail=f"{text!r} for class {c.class_id}; published name is {want_de!r}")
