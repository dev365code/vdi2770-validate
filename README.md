...
| GitHub Action | a workflow that blocks a bad delivery | `uses: dev365code/vdi2770-validate@v0.9.5` |
...
```yaml
- uses: dev365code/vdi2770-validate@v0.9.5
  with:
...
| `version` | which release to install. Left empty it is the ref you pinned — `@v0.9.5` installs 0.9.5 — falling back to the version the action's own checkout publishes, so `@main` may name a version not on the index yet. **The rules travel with the engine**: an older `version` is an older rule set and may return a different verdict. Four releases cannot be asked for at all: 0.8.0, 0.8.1, 0.8.2 and 0.9.0 name their engine with a floor, so installing one brings a newer engine beside it and the tool refuses to judge a pair that disagrees with itself (exit 3). Ask for 0.9.5 or later: every earlier release is inside the range of GHSA-6hqr-phm3-chpf |
...
**A patch release repairs — and three of the eight patch releases of this package
...
The other five moved no verdict at all: `0.9.1`; `0.9.2`, which repaired what
...
rather than in the characters it stores; and `0.9.5`, which keeps every
sentence of the text report on its own line and charges the budget for the
console that prints most.
...
`vdi2770-validate==0.9.5` installs one matching pair.
...
exists. Move to 0.9.5 or later, which pins both halves exactly — and pinning
...
```
pip install "vdi2770-validate==0.9.5"
```

Move there from any release, not only those four. Every release up to 0.9.4
is inside GHSA-6hqr-phm3-chpf and GHSA-3pfq-57fx-w4q5, and every one before
0.9.2 is inside another advisory as well — GHSA-xp97-jcmj-h45f reaches every
release up to 0.8.0, and GHSA-f9xw-89gp-x52p every release from 0.8.0 to 0.9.1
— so 0.9.5 is the first release outside all four. The pin runs one way: the engine does not
