...
with no arguments lists the table; it holds 176 rows, each naming the pytest
selection or the tool that has to go red.

`make standalone` runs each of the 86 test files on its own. A suite is a shared
