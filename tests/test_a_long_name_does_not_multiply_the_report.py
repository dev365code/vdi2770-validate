...
def _held(report, rid):
    """Bytes of listing a report holds for one rule, counted the way the budget
    counts them -- the same on every interpreter, as a peak is not."""
...
def test_a_stopped_listing_holds_its_budget_and_no_more(tmp_path):
    ...
    for containers in (10, 20):
        ...
        assert LISTING_BUDGET_PER_RULE - 2 * one < held <= LISTING_BUDGET_PER_RULE, (
            f"{containers} containers: M10 holds {held:,} bytes against a "
...
OBJECT = '<ObjectId RefType="product type" ObjectType="Type">BR-01</ObjectId>'


def _identifiers_under_a_long_name(directory, containers, name_len, per_window, window=5):
    """`M13`'s shape of the same input: for each run of `window` consecutive
    document containers, `per_window` identifiers that those containers declare
    as alternating kinds. Every identifier is one `M13` finding whose detail
    names the containers it is declared in -- each path beginning with the long
    name -- and each run starts in a different container, so the hundred per
    rule per container never engages."""
    with zipfile.ZipFile(CORPUS / "container" / "documentcontainer.zip") as z:
        meta = next(n for n in z.namelist() if n.endswith("VDI2770_Metadata.xml"))
        text = z.read(meta).decode("utf-8")
    assert text.count(OBJECT) == 1, "the sample no longer declares its object once"

    def child(i):
        kind = "Individual" if i % 2 else "Type"
        runs = [j for j in range(containers - window + 1) if j <= i < j + window]
    ...
    with zipfile.ZipFile(middle, "w", zipfile.ZIP_STORED) as z:
        for i in range(containers):
            z.writestr(f"c{i:04d}.zip", child(i))
    outer = io.BytesIO()
    with zipfile.ZipFile(outer, "w", zipfile.ZIP_STORED) as z:
        z.writestr("M" * name_len + ".zip", middle.getvalue())
    directory.mkdir(parents=True, exist_ok=True)
    path = directory / f"ids-{containers}.zip"
    path.write_bytes(outer.getvalue())
    return str(path)
...
def test_m13_holds_the_same_budget_and_no_more(tmp_path):
    ...
    for mult in (1, 2):
        ...
        assert LISTING_BUDGET_PER_RULE - 2 * one < held <= LISTING_BUDGET_PER_RULE, (
            f"a name of {mult * NAME:,}: M13 holds {held:,} bytes against a "
            f"budget of {LISTING_BUDGET_PER_RULE:,}, in findings of {one:,}")
